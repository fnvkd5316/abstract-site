...
describe('Site 요구사항 테스트', () => {
    ...
    test('하나의 Site에 동일한 이름의 Board를 추가할 수 없다.', () => {
        ...
        mySite.addBoard(new Board('공지사항'));

        expect(() => mySite.addBoard(new Board('공지사항'))).toThrow('board이름 중복');
        expect(() => mySite.addBoard(new Board('공지사항1'))).not.toThrow();
        expect(() => mySite.addBoard(new Board('공지2사항'))).not.toThrow();
        expect(() => mySite.addBoard(new Board('공지#사항'))).not.toThrow();
        expect(() => mySite.addBoard(new Board('공지!사항!'))).not.toThrow();
        expect(() => mySite.addBoard(new Board('공지사항@@'))).not.toThrow();        
    });
    ...
    test('Board는 n개 이상 추가 할 수 있다.', () => {
        ...
        const qaBoard = new Board('QA');
        const imgBoard = new Board('image');

        expect(() => {
            mySite.addBoard(noticeBoard);
            mySite.addBoard(faqBoard);
            mySite.addBoard(qaBoard);
            mySite.addBoard(imgBoard);
        }).not.toThrow();

        expect(mySite.boards).toEqual([noticeBoard, faqBoard, qaBoard, imgBoard]);
    });
});
...
describe('Board 요구사항 테스트', () => {
    ...
    */
    ...
    test('Board에 Article을 추가할 때 Article에 ID를 자동 생성해서 부여해야 한다.', () => {
        ...
        expect(article.id.startsWith('공지사핫-')).toBe(false);
    });
